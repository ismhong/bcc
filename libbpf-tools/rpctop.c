...
"\n"
"EXAMPLES:\n"
"    ./rpctop            # trace Realtek remote procedure call\n"
"    ./rpctop -C         # don't clear the screen\n"
"    ./rpctop 5          # 5 second summaries\n"
"    ./rpctop 5 10       # 5 second summaries, 10 times only\n"
"    ./rpctop -f         # also display versionID, parameterSize, mycontext\n"
...
static struct id_name procedureID_a_system2agent[] = {
	...
	{5040, "VIDEO_RPC_SUBPIC_DEC_ToAgent_ConfigureResolution"}, {5050, "VIDEO_RPC_ToAgent_Get_EDID_Data"},
...
int main(int argc, char **argv)
{
	...
	if (env.filter_us_latency)
		skel->rodata->min_us_latency = env.filter_us_latency;
	...
	err = rpctop_bpf__load(skel);
	if (err) {
		fprintf(stderr, "Failed to load and verify BPF skeleton\n");
		goto cleanup;
	}

	err = rpctop_bpf__attach(skel);
	if (err) {
	...
	while (true) {
		...
		if (env.fulldisplay) {
			printf("%-14s %6s %6s %6s %1s %12s %9s %60s %13s %13s %12s %13s %18s\n",
				"COMM", "TGID", "PID", "P_TYPE", "B",
				"programID", "versionID", "procedureID", "parameterSize",
				"mycontext", "duration(us)", "timestamp(us)",
				use_rpc_mode ? "90K_clk" : "BPF_time(ns)");
		} else {
			printf("%-14s %6s %6s %6s %1s %12s %60s %12s %13s %18s\n",
				"COMM", "TGID", "PID", "P_TYPE", "B", "programID",
				"procedureID", "duration(us)", "timestamp(us)",
				use_rpc_mode ? "90K_clk" : "BPF_time(ns)");
		}

		int map_fd = bpf_map__fd(skel->maps.done);
		__u32 i = 0;
		int event_count = 0;
		__u64 lookup_key = -1, next_key;

		__u64 collected_keys[10240];
		int key_count = 0;
		while (bpf_map_get_next_key(map_fd, &lookup_key, &next_key) == 0) {
			collected_keys[key_count++] = next_key;
			lookup_key = next_key;
		}

		for (i = 0; i < key_count; i++) {
			if (bpf_map_lookup_elem(map_fd, &collected_keys[i], &events[event_count]) == 0) {
				event_count++;
			}
		}

		for (i = 0; i < key_count; i++) {
			bpf_map_delete_elem(map_fd, &collected_keys[i]);
		}
		...
		for (i = 0; i < event_count; i++) {
			...
			if (use_rpc_mode) {
				current_timestamp = e->refclk * 1000000 / 90000;
			} else {
				current_timestamp = e->refclk / 1000;
			}
			if (time_base == 0)
				time_base = current_timestamp;

			if (env.fulldisplay) {
				printf("%-14.14s %6d %6d %6s %1s %12s %9u %60s %13u %13u %12.3f %13llu %18llu\n",
					e->comm, e->tgid, e->pid, e->rpc_num ? "user" : "kernel",
					e->taskID ? "Y" : "N",
					programID_name(e->programID),
					e->versionID,
					procedureID_name(e->programID, e->procedureID, e->rpc_num, use_rpc_mode),
					e->parameterSize, e->mycontext, (double)e->delta / 1000,
					current_timestamp - time_base, e->refclk);
			} else {
				printf("%-14.14s %6d %6d %6s %1s %12s %60s %12.3f %13llu %18llu\n",
					e->comm, e->tgid, e->pid, e->rpc_num ? "user" : "kernel",
					e->taskID ? "Y" : "N",
					programID_name(e->programID),
					procedureID_name(e->programID, e->procedureID, e->rpc_num, use_rpc_mode),
