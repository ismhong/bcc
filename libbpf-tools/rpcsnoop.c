...
"\n"
"EXAMPLES:\n"
"    ./rpcsnoop           # trace Realtek remote procedure call\n"
"    ./rpcsnoop -d 10     # trace for 10 seconds only\n"
...
static int handle_event(void *ctx, void *data, size_t data_sz)
{
	...
	static __u64 time_base = 0;
	__u64 current_timestamp;
	bool use_rpc_mode = *(bool *)ctx;
	...
	if (env.fulldisplay) {
		printf("%-14.14s %6d %6d %6s %1s %12s %9u %-60s %13u %13u %14llu %18llu\n",
				...
				e->parameterSize, e->mycontext,
				current_timestamp - time_base, e->refclk);
	} else {
		printf("%-14.14s %6d %6d %6s %1s %12s %-60s %14llu %18llu\n",
				e->comm, e->tgid, e->pid, e->rpc_num ? "user" : "kernel",
				e->taskID ? "Y" : "N",
				programID_name(e->programID),
				procedureID_name(e->programID, e->procedureID, e->rpc_num, use_rpc_mode),
...
int main(int argc, char **argv)
{
	...
	err = rpcsnoop_bpf__load(skel);
	if (err) {
		fprintf(stderr, "Failed to load and verify BPF skeleton\n");
		goto cleanup;
	}

	err = rpcsnoop_bpf__attach(skel);
	if (err) {
		fprintf(stderr, "Failed to attach BPF skeleton\n");
		goto cleanup;
	}

	rb = ring_buffer__new(bpf_map__fd(skel->maps.events), handle_event, &use_rpc_mode, NULL);
	if (!rb) {
		err = -1;
	...
	printf("===============================================================\n");
	printf("WARNING: The rpcsnoop results are not guaranteed to be in order\n");
	printf("===============================================================\n");

	if (env.fulldisplay) {
		printf("%-14s %6s %6s %6s %1s %12s %9s %-60s %13s %13s %14s %18s\n",
				"COMM", "TGID", "PID", "P_TYPE", "B", "programID", "versionID", "procedureID", "parameterSize",
				"mycontext", "timestamp(us)",
				use_rpc_mode ? "90K_clk" : "BPF_time(ns)");
	} else {
		printf("%-14s %6s %6s %6s %1s %12s %-60s %14s %18s\n",
				"COMM", "TGID", "PID", "P_TYPE", "B", "programID",
				"procedureID", "timestamp(us)",
				use_rpc_mode ? "90K_clk" : "BPF_time(ns)");
	}
	...
	while (true) {
		...
		if (env.duration && (time(NULL) - start_ts) >= env.duration)
			break;
	}

cleanup:
	ring_buffer__free(rb);
	rpcsnoop_bpf__destroy(skel);
	return -err;
}
